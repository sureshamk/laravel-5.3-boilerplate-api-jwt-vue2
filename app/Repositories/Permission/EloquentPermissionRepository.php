...
use App\Repositories\Permission\Dependency\PermissionDependencyRepositoryContract;
...
class EloquentPermissionRepository implements PermissionRepositoryContract
{
    ...
    public function findOrThrowException($id, $withRoles = false)
    {
        if (!is_null(Permission::find($id))) {
            if ($withRoles) {
                return Permission::with('roles')->with('dependencies')->find($id);
            }
    ...
    {
        return Permission::with('roles')->with('dependencies')->orderBy($order_by, $sort)->paginate($per_page);
    }
    ...
    public function create($input, $roles)
    {
        ...
        $permission->group_id = isset($input['group_id']) && strlen($input['group_id']) > 0 ? (int) $input['group_id'] : null;
        ...
        if ($permission->save()) {
            ...
            if (count($roles['roles']) > 0) {
                foreach ($roles['roles'] as $role_id) {
